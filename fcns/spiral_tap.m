function ximage = spiral_tap(lr, lc, bincount, y, A, tau, psf, jit, mtx, miniter, maxiter, varargin)
...
Axprevious = Ax;
xprevious = x;
grad = computegrad(x,y,lr,lc,bincount,A,Ax,tau,noisetype,logepsilon,penalty,psf,jit,mtx);
...
while (iter <= miniter) || ((iter <= maxiter) && not(converged))
    ...
    xoutput = x + mu;
    ref_recover=zeros(lr,lc);
    dep_recover=zeros(lr,lc);
    for a=1:lr
        for b=1:lc
            ref_recover(a,b)= max(xoutput(a,b,:));
            for vn = 1:bincount
            dep_recover(a,b) = dep_recover(a,b)+xoutput(a,b,vn)*vn;
            end
           dep_recover(a,b) = dep_recover(a,b)/sum(xoutput(a,b,:));
        end
    end
    
    for i = 1 : size(xoutput,3)
        aaa(:,:) = xoutput(:,:,i);
        aaa = fliplr(aaa);
        xoutput(:,:,i) = aaa;
    end
    pd = permute(xoutput,[3 2 1]);             % convert signal to t domain
    mpd = reshape(mtx'*pd(:,:),[bincount lr lc]);
    ximage = permute(mpd,[3 2 1]);
    ximage = permute(ximage, [3 1 2]);
end
% ===========================
...
function grad2 = computegrad(x,y,lr,lc,bincount,A,Ax,tau,noisetype,logepsilon,...
    penalty,psf,jit,mtx)
    ...
    switch lower(noisetype)
        case 'poisson'
            ...
            B = y./(Ax+logepsilon);
            CB = transf(B);
            blur = flip(flip(flip(psf,1),2),3);
            grad2 = juanji(count,blur);
            grad2 = grad2-juanji(CB,blur);
        case 'gaussian'
            B = y-Ax;
            %B = convn(B,jit,'same');
            pB = permute(B,[3 2 1]);
            mpB = reshape(mtx*pB(:,:),[bincount lr lc]);
            CB = permute(mpB,[3 2 1]);
            blur = flip(flip(flip(psf,1),2),3);
            grad2 = -1*juanji(blur,CB,2);
            %grad2 = -1*convn(CB,blur,'same');
    end
    %}
end
...
function subsolution = computesubsolution(step,tau,alpha,penalty,mu,varargin)
    switch lower(penalty)
        ...
        case 'tv'
            ...
            if tau>0
                subsolution = denoise_bound_3D(step,tau./alpha,-mu,Inf,pars);
            else
...
function [ sb ] = juanji( big ,small, a )
    [yb,xb,zb] = size(big);
    [ys,xs,zs] = size(small);
    pada = zeros(yb,xb,zb);
    ...
    pada(qq:ww,ee:rr,tt:yy) = small;
    fs = ifftshift(fftn(fftshift(pada)));  
    fb = ifftshift(fftn(fftshift(big)));
    if a == 1
