...
function [x, varargout] = SPIRALTAPv1(lr, lc, bincount, y, A, tau, varargin)
...
while (iter <= miniter) || ((iter <= maxiter) && not(converged))
    ...
    for a=1:lr
        for b=1:lc
%             for c=1:bincount
%                 dep_recover(a,b)=dep_recover(a,b)+c*xoutput(a,b,c);
%             end
            dep_recover(a,b)=max(xoutput(a,b,:));
%             if dep_recover(a,b)~=0
               %dep_recover(a,b)=dep_recover(a,b)/sum(xoutput(a,b,:));
               ref_recover(a,b)=max(xoutput(a,b,:));
%             end
        end
    end
    dep_recover = dep_recover*1.5/10.0;
    %dep_recover = dep_recover*bin*10^(12)+mm;
    save(['./结果与结果图/','tv_',num2str(tau),'_','iter_',num2str(iter-1),'.mat'],'ref_recover','dep_recover');
    namefig=figure('Name',['tv_',num2str(tau),'&','iter_',num2str(iter-1)],'NumberTitle','off');
    subplot(1,2,1);imshow(ref_recover,[]);colorbar;title('RML-spiral_{ref}');
    subplot(1,2,2);imagesc(dep_recover);axis image;colorbar;colormap('jet');title('RML-spiral_{dep}');
    saveas(namefig,['./结果与结果图/','tv_',num2str(tau),'&','iter_',num2str(iter-1),'.fig']);
    saveas(namefig,['./结果与结果图/','tv_',num2str(tau),'&','iter_',num2str(iter-1),'.png']);
    %close(namefig);
end
% ===========================
